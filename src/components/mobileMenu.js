...
const MobileMenu = () => {
  ...
  return (
    <div className="relative flex flex-row-reverse p-2 ">
      ...
      <div
        ref={menuRef}
        className={`absolute  w-28 inset-y-11 -right-16 bg-white shadow-lg rounded-md overflow-hidden transition-all duration-300 ease-in-out transform ${
          isOpen
            ? "opacity-100 scale-100 translate-y-0 visible h-screen  " : "opacity-0 scale-95 -translate-y-3 invisible"
        }`}
      >
        <ul className="flex flex-col  justify-self-end space-y-2 p-4   text-gray-800 font-serif">
          {["Home", "About", "Services", "Contact", "Vision", "Mission", "Updates"].map(
            (item) => (
              <li
                key={item}
